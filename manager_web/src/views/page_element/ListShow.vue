<template>
    <div id="listShow">
        <div class="upload-container">
            <Upload @getImgUrl3="initImgUrl" :source="this.$options.name" ref="upload"></Upload>
        </div>
        ...
        <div class="table">
            <el-divider content-position="left">已添加的图文信息</el-divider>
            <div>
                <ListShowTable ref="listShowTable" :page-type="pageType" :group-id="groupId"></ListShowTable>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    import ListShowTable from '../components/ListShowTable'
    import SelfTinymceEditor from '../../components/tinymce/SelfTinymceEditor'
    export default {
        name: 'ListShow',
        components: {Upload, ListShowTable, SelfTinymceEditor},
        props: {
            pageType: {
                type: Number
            },
        ...
        methods: {
            ...
            submit() {
                const url = this.commRouter + '/listShow/save';
                ...
                this.httpPost(url, data, true, true)
                    .then((response) => {
                        ...
                        this.$refs.listShowTable.getData(1);
                    }).catch((error) => {
                    console.log(error)
                });
            },
